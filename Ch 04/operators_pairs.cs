// created on 12/24/2002 at 1:05 PM
...
public struct Fraction
{
  ...
  public static bool operator>(Fraction lh_fraction, Fraction rh_fraction)
{
  int lowest;

  lowest = lcd(lh_fraction.Denominator,
    rh_fraction.Denominator);
    
  if (lh_fraction.Numerator *
    (lowest / lh_fraction.Denominator) >
    rh_fraction.Numerator *
    (lowest / rh_fraction.Denominator))
    return true;
  else
    return false;
}
...
public static bool operator<(Fraction lh_fraction, Fraction rh_fraction)
{
  return !(lh_fraction > rh_fraction);
}
...
class FractionCalculator
{
  static void Main()
  {
    ...
    Console.WriteLine("Fraction 1 > Fraction 2? " + (fraction1 > fraction2).ToString());
    Console.WriteLine("Fraction 1 < Fraction 2? " + (fraction1 < fraction2).ToString());
    Console.WriteLine("Fraction 2 > Fraction 1? " + (fraction2 > fraction1).ToString());
    Console.WriteLine("Fraction 2 < Fraction 1? " + (fraction2 < fraction1).ToString());
  }
} 
